...
void amin::ability2(Hero* ptTeam[] , Hero* ptEnemy[])
{
    ...
    cout << "   > Take 25 HP from a team mate & heal self by 75 HP" << endl;

    bool checkTarget = false;

    for(int i = 0; i < 3; i++)
    {
        if(ptTeam[i]->checkalive() && ptTeam[i] != this) 
            checkTarget = true;
    }
...
void amin::special_ablity(Hero* ptTeam[] , Hero* ptEnemy[])
{
    ...
    if(checkTarget)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (ptTeam[i] != this)
            {
                teamhp = ptTeam[i]->get_hp();
                teamhp -= 30;
                if(teamhp < 0) teamhp = 0;
                ptTeam[i]->set_hp(teamhp);
